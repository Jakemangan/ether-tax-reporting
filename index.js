...
async function processTxHash(txHash){
    ...
    */
    ...
    if(type === "BUY"){
        ...
        if(tokenEvent.topics[1] !== myWalletNonHex){
            throw new Error("BUY transaction token log topic #2 is not equal to wallet address");
        }
    } else {
        ...
        if(tokenEvent.topics[1] !== myWalletNonHex){
            throw new Error("SELL transaction token log topic #2 is not equal to wallet address");
        }
    }
